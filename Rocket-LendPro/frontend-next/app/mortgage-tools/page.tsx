...
export default function MortgageCalculatorPage() {
  ...
  // Get property price from URL if available
  const propertyPriceFromUrl = searchParams.get('propertyPrice');
  const defaultPropertyValue = propertyPriceFromUrl ? parseFloat(propertyPriceFromUrl) : (currentCalculation?.propertyValue || 360000);
  const defaultDownPayment = propertyPriceFromUrl ? parseFloat(propertyPriceFromUrl) * 0.2 : (currentCalculation?.downPayment || 60000); // 20% down by default
  const defaultLoanAmount = propertyPriceFromUrl ? parseFloat(propertyPriceFromUrl) * 0.8 : (currentCalculation?.loanAmount || 300000);
  ...
  // Update property value when loan amount and down payment change
  useEffect(() => {
            ...
            <>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <PaymentBreakdownChart
                  principal={calculationResult.amortizationSchedule[0]?.principal || calculationResult.principalAndInterest * 0.3}
                  interest={calculationResult.amortizationSchedule[0]?.interest || calculationResult.principalAndInterest * 0.7}
