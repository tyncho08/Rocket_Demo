...
export default function MarketTrendsPage() {
  ...
  const filteredPriceHistory = useMemo(() => {
    if (selectedTimeframe === '6m') {
      ...
      return priceHistory;
    } else if (selectedTimeframe === '2y') {
      // For demo, just repeat the data
      return [...priceHistory, ...priceHistory];
    } else {
      // For 5y, repeat multiple times
      return [...priceHistory, ...priceHistory, ...priceHistory, ...priceHistory, ...priceHistory];
    }
  }, [selectedTimeframe, priceHistory]);
  
  // Filter regional data based on selected region
  const filteredRegional = useMemo(() => {
    if (selectedRegion === 'national') {
      return regional;
    } else if (selectedRegion === 'texas') {
      return regional;
    } else {
      ...
      return regional.filter(city => city.city === cityMap[selectedRegion]);
    }
  ...
  const exportReport = () => {
    try {
      const reportData = {
        ...
        region: regionOptions.find(r => r.value === selectedRegion)?.label || selectedRegion,
        timeframe: timeframeOptions.find(t => t.value === selectedTimeframe)?.label || selectedTimeframe,
        marketData: {
          ...
          daysOnMarket: national.daysOnMarket
        },
      ...
      const csvRows = [
        ...
        ['Key Metrics'],
        ['Metric', 'Value', 'Change'],
        ['Median Home Price', `$${national.medianHomePrice}`, `${national.priceChange}%`],
        ['Average Rate', `${national.averageRate}%`, `${national.rateChange}%`],
        ['Inventory (Months)', national.inventory, `${national.inventoryChange}%`],
        ['Days on Market', national.daysOnMarket, `${national.domChange} days`],
        [],
        ['Regional Data'],
        ['City', 'Median Price', 'Price Change', 'Inventory', 'Days on Market', 'Hot Score'],
        ...filteredRegional.map(city => [
          city.city,
          `$${city.medianPrice}`,
          `${city.priceChange}%`,
          `${city.inventory} mo`,
          `${city.daysOnMarket} days`,
          city.hotScore
        ])
      ];
      ...
      link.setAttribute('download', `market-report-${selectedRegion}-${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {activeTab === 'overview' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <MarketOverviewChart data={filteredPriceHistory} />
          ...
          {activeTab === 'rates' && (
            <div className="space-y-6">
              <InterestRateChart data={filteredPriceHistory} />
          ...
          {activeTab === 'regional' && (
            <div className="space-y-6">
              <RegionalComparisonChart data={filteredRegional} />
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {filteredRegional.map((city, index) => (
                  <Card key={index}>
                    <CardContent className="pt-6">
                      <div className="mb-4">
